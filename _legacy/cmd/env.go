...
var envUpdateCmd = &cobra.Command{
	...
	Long:  "Modify an environment with arbitrary fields. Use --field key=val (repeatable).",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		env, err := apiClient.UpdateEnvironment(args[0], params)
		if err != nil {
			return err
		}
		return output.Print(cfg.Output, env)
...
func init() {
	envUpdateCmd.Flags().StringSliceP("field", "F", nil, "key=value (repeatable)")
