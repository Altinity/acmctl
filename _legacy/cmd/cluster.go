...
var clusterTempCredsCmd = &cobra.Command{
	Use:   "temp-creds <id>",
	Short: "Generate a temporary user with Altinity support permissions",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		creds, err := apiClient.GetSupportCredentials(args[0])
		if err != nil {
			return err
		}
		return output.Print("json", creds)
	},
}

var clusterCHAPIListCmd = &cobra.Command{
	Use:   "ch-api-endpoints <id>",
	Short: "List CH API endpoints for a cluster",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		result, err := apiClient.ListCHAPIEndpoints(args[0])
		if err != nil {
			return err
		}
...
var clusterCHAPIUpdateCmd = &cobra.Command{
	Use:   "ch-api-endpoints-update <id>",
	Short: "Update CH API endpoints for a cluster",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		val, _ := cmd.Flags().GetString("endpoints")
		resolved, err := resolveValue(val)
		if err != nil {
			return err
		}
		result, err := apiClient.UpdateCHAPIEndpoints(args[0], map[string]string{"chAPIEndpoints": resolved})
		if err != nil {
			return err
		}
		return printJSON(result)
...
var clusterCHAPIRunCmd = &cobra.Command{
	Use:   "ch-api-run <id>",
	Short: "Run a query via CH API endpoint",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		result, err := apiClient.RunCHAPIEndpoint(args[0], params)
		if err != nil {
			return err
		}
		return printJSON(result)
...
var clusterUpdateCmd = &cobra.Command{
	...
	Long:  "Modify a cluster with arbitrary fields. Use --field key=val (repeatable).",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		cluster, err := apiClient.UpdateCluster(args[0], params)
		if err != nil {
			return err
		}
		return output.Print(cfg.Output, cluster)
...
var clusterLogsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		params := flagsToParams(cmd, map[string]string{
			"node": "node", "count": "count", "filter": "filter",
			"download": "download", "section": "section",
		})
		result, err := apiClient.GetClusterLogs(args[0], params)
		if err != nil {
			return err
		}
		return output.Print("json", result)
...
var clusterProfilesCmd = &cobra.Command{
	Use:   "profiles <id>",
	Short: "List cluster settings profiles",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		profiles, err := apiClient.ListClusterProfiles(args[0])
		if err != nil {
			return err
		}
		return output.PrintTabulableList(cfg.Output, profiles)
	},
}

var clusterRefreshSupportCmd = &cobra.Command{
	Use:   "refresh-support <id>",
	Short: "Refresh Altinity support access",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		if err := apiClient.RefreshSupportAccess(args[0]); err != nil {
			return err
		}
		fmt.Printf("Cluster %s support access refreshed.\n", args[0])
		return nil
	},
}
...
func init() {
	...
	// update
	clusterUpdateCmd.Flags().StringSliceP("field", "F", nil, "key=value (repeatable)")

	// ch-api endpoints
	clusterCHAPIUpdateCmd.Flags().String("endpoints", "", "endpoints definition (use @file or @- for JSON)")
	for _, name := range []string{"endpoint", "method", "parameters", "user", "password"} {
		clusterCHAPIRunCmd.Flags().String(name, "", "")
	}

	// logs
	clusterLogsCmd.Flags().String("node", "", "target node")
	clusterLogsCmd.Flags().String("count", "", "max records")
	clusterLogsCmd.Flags().String("filter", "", "filter expression")
	clusterLogsCmd.Flags().String("download", "", "download mode")
	clusterLogsCmd.Flags().String("section", "", "log section")
	...
	clusterCmd.AddCommand(clusterRefreshSupportCmd)
	clusterCmd.AddCommand(clusterUpdateCmd)
	clusterCmd.AddCommand(clusterLogsCmd)
	clusterCmd.AddCommand(clusterProfilesCmd)
	clusterCmd.AddCommand(clusterCHAPIListCmd)
	clusterCmd.AddCommand(clusterCHAPIUpdateCmd)
	clusterCmd.AddCommand(clusterCHAPIRunCmd)
